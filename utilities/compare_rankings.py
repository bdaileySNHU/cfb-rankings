...
def compare_with_ap_poll():
    ...
    if unique_picks:
        ...
        for r in unique_picks:
            print(
                f"#{r['rank']:<7} {r['team_name']:<25} {r['wins']}-{r['losses']:<8} {r['elo_rating']:<10.2f} {r['sos']:<10.2f}"
            )
    else:
...
def analyze_prediction_accuracy():
    ...
    for game in games:
        ...
        home_rating_adj = (
            home_rating_before + 65 if not game.is_neutral_site else home_rating_before
        )
        ...
        if predicted_winner.id == actual_winner.id:
            ...
            score = (
                ...
                if actual_winner.id == home_team.id
                else f"{game.away_score}-{game.home_score}"
            )
...
def show_elo_insights():
    ...
    changes = [
        (t.name, t.elo_rating - t.initial_rating, f"{t.wins}-{t.losses}", t.conference.value)
        for t in teams
    ...
    for i, r in enumerate(sos_leaders, 1):
        print(
            f"{i}. {r['team_name']:<25} SOS: {r['sos']:.1f}  (Rank #{r['rank']}, {r['wins']}-{r['losses']})"
        )
