...
@pytest.mark.unit
class TestQuarterScoresDoNotAffectRatings:
    """Quarter scores are stored for reference only.

    They used to feed a quarter-weighted MOV that discounted garbage-time
    scoring, but backtesting five seasons showed it cost accuracy and Brier
    score, so ratings use the final margin. These guard against the branch
    coming back by accident.
    """

    def test_quarter_scores_do_not_change_the_mov(self, test_db: Session):
        service = RankingService(test_db)
        ...
        # Home led 35-7 after three quarters, so the old code called this
        # garbage time and discounted the fourth, landing on ~1.85. The margin
        # alone caps out instead.
        game = Game(
            home_team_id=home_team.id,
            away_team_id=away_team.id,
            home_score=49,
        ...
        assert game.is_processed is True
        assert result["mov_multiplier"] == RankingService.MAX_MOV_MULTIPLIER

    def test_mov_still_follows_the_log_of_the_margin(self, test_db: Session):
        service = RankingService(test_db)
        home_team = Team(name="Home", conference=ConferenceType.POWER_5, elo_rating=1600.0)
        away_team = Team(name="Away", conference=ConferenceType.POWER_5, elo_rating=1600.0)
        test_db.add_all([home_team, away_team])
        ...
        assert abs(result["mov_multiplier"] - math.log(7 + 1)) < 0.01
